...
Функция ФайлПовисшейЗадачи(ФайлДляРазбора)
    ...
    ИнформацияПоCPU = Новый РегулярноеВыражение("Node,(WorkingSetSize|PercentProcessorTime)\s\S*,([0-9]*)"); 

    Текст = Новый ЧтениеТекста(ФайлДляРазбора.ПолноеИмя);
    Совпадения = ИнформацияПоCPU.НайтиСовпадения(Текст.Прочитать());
    МассивЗагрузкаCPU = Новый Массив;
    МассивЗагрузкаRAM = Новый Массив;
    Итератор = 0;
    Пока Итератор < Совпадения.Количество() Цикл       
        
        //МассивЗагрузкаCPU
        //Сообщить("" +  Совпадения[Итератор].Группы[2].Значение + "--" + Совпадения[Итератор+1].Группы[2].Значение);
        МассивЗагрузкаCPU.Добавить(Совпадения[Итератор].Группы[2].Значение);
        МассивЗагрузкаRAM.Добавить(Совпадения[Итератор+1].Группы[2].Значение);
        Итератор = Итератор + 2;

    КонецЦикла;

    Если МассивЗагрузкаCPU.Количество() <> МассивЗагрузкаRAM.Количество() Тогда

        Лог.Ошибка("Информация по активности процессов не полна. Проверьте файлы pidов, или перезапустите скрипт.");

    Иначе

        Лог.Отладка("Проверяем дальше!");
        ПроверитьМассив(МассивЗагрузкаCPU);
...
Процедура ПроверитьМассив(МассивЗначенийПараметраПид)

    ПоследниеЗначение = МассивЗначенийПараметраПид[МассивЗначенийПараметраПид.Количество() - 1];

    Для каждого Элемент Из МассивЗначенийПараметраПид Цикл
